...
if ($saldo <= $cliente_saldo) {
  $resta = $cliente_saldo - $saldo;
  $resta = round($resta,2);
  $motivo =  'CONVERSION DE SALDO '.$rs_cliente['TX_cliente_nombre'];
  $link->query("UPDATE bh_cliente SET TX_cliente_saldo = '$resta' WHERE AI_cliente_id = '$client_id'")or die($link->error);
  ...
  VALUES ('$user_id', '{$rs_impresoraid['AI_impresora_id']}', 'SALIDA', '$motivo', '$saldo', '$fecha_actual', 'ACTIVA')")or die($link->error);
}
