...
$link->query("INSERT INTO bh_pedido (pedido_AI_user_id,pedido_AI_proveedor_id, TX_pedido_fecha, TX_pedido_status) VALUES ('{$_COOKIE['coo_iuser']}','$provider','$fecha_actual','ACTIVO')")or die($link->error);

$qry_lastid = $link->query("SELECT LAST_INSERT_ID();");
$rs_lastid = $qry_lastid->fetch_array();
$last_id = trim($rs_lastid[0]);

$numero = "000000000".$last_id;
$numero = substr($numero, -10);

$link->query("UPDATE bh_pedido SET TX_pedido_numero='$numero' WHERE AI_pedido_id = '$last_id'")or die($link->error);
$raw_toinsert=array();
$i=0;
foreach($raw_product as $index => $product){
  if(!empty($product)){
    $raw_toinsert[$i]=$product;
    $i++;
  }
}
//echo json_encode($raw_toinsert);
 foreach ($raw_toinsert as $key => $value) {
     $link->query("INSERT INTO bh_datopedido (datopedido_AI_pedido_id, datopedido_AI_producto_id, TX_datopedido_cantidad, TX_datopedido_precio) VALUES ('$last_id','{$value['id']}','{$value['cantidad']}','{$value['precio']}')")or die($link-error);
 }
