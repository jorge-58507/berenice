...
function ins_datocompra($factura_id,$product,$cantidad,$precio,$itbm,$descuento,$p4,$preguardado,$medida){
	...
	$fecha_actual = date('Y-m-d');
							//  ############ LLAMANDO STOCK
	$qry_product=$link->query("SELECT TX_producto_cantidad, TX_producto_medida FROM bh_producto WHERE AI_producto_id = '$product'");
	...
	if($qry_precio->num_rows < 1 && $p4 > 0.00 && $preguardado < 1){
		...
		$txt_insert_precio="INSERT INTO bh_precio (precio_AI_producto_id, precio_AI_medida_id, TX_precio_cuatro, TX_precio_fecha ) VALUES ('$product','{$rs_product['TX_producto_medida']}','$p4','$fecha_actual')";
