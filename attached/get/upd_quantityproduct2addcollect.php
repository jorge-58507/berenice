...
$new_quantity=$_GET['b'];
	...
	$qry_datoventa=$link->query("SELECT * FROM bh_datoventa WHERE AI_datoventa_id = '$datoventa_id'")or die($link->error);
	if($qry_datoventa->num_rows > 0){
...
foreach ($arr_factid as $key => $value) {
	$txt_clientid .= ($value === end($arr_factid)) ?	" AI_facturaventa_id = '$value'" : " AI_facturaventa_id = '$value' OR";
}
