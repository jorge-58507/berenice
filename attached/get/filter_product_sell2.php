	...
	$i++;
			...
			if($nr_product=$qry_product->num_rows > 0){
				foreach ($raw_producto as $key => $rs_product) {
					$style = ($rs_product['TX_producto_inventariado'] === '1') ? 'style="background-color: #ecffe4";' : '';
					$title = ($rs_product['TX_producto_inventariado'] === '1') ? 'INCLUIDO' : 'NO INCLUIDO'; ?>
			    <tr onclick="javascript:open_product2sell(<?php echo $rs_product['AI_producto_id']; ?>);" title="<?php echo $title; ?>" <?php echo $style; ?> >
