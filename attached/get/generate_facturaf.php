...
if ($qry_impresora->num_rows < 1) {	return false;	}
$rs_impresora=$qry_impresora->fetch_array();
$impresora_id = $rs_impresora['AI_impresora_id'];
...
/* ^########################### FUNCIONES ##################### ^ */
$subtotal_ni=0;	$subtotal_ci=0;	$impuesto=0;	$descuento_ni=0;	$descuento_ci=0;
...
foreach ($raw_facturaventa as $key => $value) {
	$qry_producto = $link->query("SELECT TX_producto_cantidad, TX_producto_descontable FROM bh_producto WHERE AI_producto_id = '{$value['datoventa_AI_producto_id']}'")or die($link->error);
	...
	if($rs_producto['TX_producto_descontable'] === '1'){
