...
$txt_cpp="SELECT bh_cpp.AI_cpp_id, bh_cpp.TX_cpp_total, bh_cpp.TX_cpp_saldo, bh_cpp.TX_cpp_fecha FROM (bh_cpp INNER JOIN bh_proveedor ON bh_proveedor.AI_proveedor_id = cpp_AI_proveedor_id) WHERE ";
foreach ($raw_value as $key => $value) {
	if ($value === end($raw_value)) {
		$txt_cpp .= " TX_cpp_total LIKE '%$value%' AND TX_cpp_fecha = '$fecha_i'";
	}else {
		$txt_cpp .= " TX_cpp_total LIKE '%$value%' AND TX_cpp_fecha = '$fecha_i' OR";
	}
}
$qry_cpp=$link->query($txt_cpp." ORDER BY TX_cpp_fecha ASC");
