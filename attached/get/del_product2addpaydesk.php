...
$product_id=$_GET['a'];
		...
		$qry_checkdatoventa=mysql_query("SELECT * FROM bh_datoventa WHERE datoventa_AI_producto_id = '$product_id'", $link);
		...
		if($nr_checkdatoventa >= 1){
			...
			$qry_checkproducto=mysql_query("SELECT * FROM bh_producto WHERE AI_producto_id = '$product_id'", $link);
			$rs_checkproducto=mysql_fetch_assoc($qry_checkproducto);
			$existencia=$rs_checkproducto['TX_producto_cantidad'];
			$cantidad=$rs_checkdatoventa['TX_datoventa_cantidad'];
			$suma=$existencia+$cantidad;
			$bh_update="UPDATE bh_producto SET TX_producto_cantidad='$suma' WHERE AI_producto_id = '$product_id'";
