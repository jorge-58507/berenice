...
$link->query("INSERT INTO bh_cpp (TX_cpp_total, TX_cpp_saldo, TX_cpp_fecha, cpp_AI_proveedor_id, cpp_AI_user_id, cpp_AI_pedido_id) VALUES ('$monto','$monto','$fecha','$proveedor_id','{$_COOKIE['coo_iuser']}','$pedido_id')")or die($link->error);

$qry_provider=$link->query("SELECT bh_proveedor.AI_proveedor_id FROM (bh_proveedor INNER JOIN bh_cpp ON bh_proveedor.AI_proveedor_id = bh_cpp.cpp_AI_proveedor_id) WHERE bh_proveedor.AI_proveedor_id = '$proveedor_id'")or die($link->error);
