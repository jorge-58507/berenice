...
$facturaf_id=$_GET['a'];
$txt_datoventa="SELECT bh_datoventa.AI_datoventa_id, bh_datoventa.datoventa_AI_producto_id, bh_datoventa.TX_datoventa_cantidad
FROM ((bh_datoventa
...
$qry_datoventa = $link->query($txt_datoventa)or die($link->error);
 ...
 while($rs_datoventa=$qry_datoventa->fetch_array()) {
   $link->query("INSERT INTO bh_nuevadevolucion
		(nuevadevolucion_AI_producto_id, nuevadevolucion_AI_datoventa_id, nuevadevolucion_AI_user_id, TX_nuevadevolucion_cantidad)
	VALUES ('{$rs_datoventa['datoventa_AI_producto_id']}','{$rs_datoventa['AI_datoventa_id']}','$user_id','{$rs_datoventa['TX_datoventa_cantidad']}')");
}
