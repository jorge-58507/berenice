...
$fecha = date('d-m-Y',strtotime($fecha_actual));
		...
		$qry_facturaf=$link->query($txt_facturaf.$line_order) or die($link->error());
			...
			while($rs_facturaf=$qry_facturaf->fetch_array(MYSQLI_ASSOC)){
				$raw_facturaf_debito[$rs_facturaf['TX_facturaf_fecha']."ff"]=$rs_facturaf;
			}
			foreach ($raw_facturaf_debito as $key => $rs_facturaf) {
				...
				while ($rs_nd=$result->fetch_array(MYSQLI_ASSOC)) {
					$raw_facturaf_debito[$rs_nd['TX_notadebito_fecha']."nd"]=$rs_nd;
				}
			}
...
<tr style="height:680px;">
    ...
    <table id="tbl_movement" class="table table-striped table-bordered">
...
<?php
	...
	$str = $key;
		if ($coincidencia = substr_count($str, "ff") > 0): ?>
			<tr>
				<td><?php echo $rs_facturaf_debito['TX_facturaf_fecha']." - ".$rs_facturaf_debito['TX_facturaf_hora']; ?></td>
				...
				<td><?php echo $sumatoria+= $rs_facturaf_debito['TX_datopago_monto']; ?></td>
			</tr>
<?php else: ?>
			<tr>
				<td><?php echo $rs_facturaf_debito['TX_notadebito_fecha']." - ".$rs_facturaf_debito['TX_notadebito_hora']; ?></td>
				...
				<td><?php $sumatoria -= $rs_facturaf_debito['TX_rel_facturafnotadebito_importe']; echo number_format($sumatoria,2); ?></td>
			</tr>
<?php endif; ?>
