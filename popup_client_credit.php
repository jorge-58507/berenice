...
$qry_client=$link->query("SELECT TX_cliente_nombre, TX_cliente_cif, TX_cliente_telefono, TX_cliente_direccion, TX_cliente_porcobrar, TX_cliente_restringido, TX_cliente_exento FROM bh_cliente WHERE AI_cliente_id = '$client_id'")or die($link->error);
...
if ($qry_oldest->num_rows > 0) {
	$ff_oldest = $rs_oldest['TX_facturaf_fecha'];
}else{
	$ff_oldest = date('Y-m-d', strtotime('-7 days'));
}
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$("#btn_save").click(function(){
				...
				var restricted = ( $('#cb_restricted').prop('checked') ) ? '1' : '0';
				var exento = ( $('#cb_exento').prop('checked') ) ? '1' : '0';

				$.ajax({
					data: {"a" : "<?php echo $client_id ?>", "b": $("#txt_limitcredit").val(), "c": $("#txt_credit_term").val(), "d" : pending, "e" : $("#txt_clientname").val(), "f" : restricted, "g" : exento},type: "GET",	dataType: "text",	url: "attached/get/upd_client_credit.php",
				})
...
/*   #########################	FACTURAS	###############################	*/
			...
			$("#btn_filter").on("click",function(){
				if($("#txt_datei").val() == "" || $("#txt_datef").val() == ""){ return false; }
				$.ajax({
				...
				.done(function( data, textStatus, jqXHR ) {	$("#container_tblfacturaf").html(data);	})
				.fail(function( jqXHR, textStatus, errorThrown ) {	console.log("BAD "+textStatus);	});
			});
			$( function() {
...
/*  #########################          PAYMENTS          ###################*/
			...
			$("#cb_restricted").attr('disabled',true);
			$("#lbl_cb_restricted").on("click", function(){		
				$.ajax({ data: "", type: "GET", dataType: "JSON", url: "attached/get/get_session_admin.php",	})
				.done(function( data, textStatus, jqXHR ) {	console.log("GOOD " + textStatus);

					if (data[0][0] != "") {
						$("#cb_restricted").attr('disabled',false);
					}
				})
				.fail(function( jqXHR, textStatus, errorThrown ) {	console.log("BAD "+textStatus);	});
			})
			$("#cb_exento").attr('disabled',true);
			$("#lbl_cb_exento").on("click", function(){		
				$.ajax({ data: "", type: "GET", dataType: "JSON", url: "attached/get/get_session_admin.php",	})
				.done(function( data, textStatus, jqXHR ) {	console.log("GOOD " + textStatus);

					if (data[0][0] != "") {
						$("#cb_exento").attr('disabled',false);
					}
				})
				.fail(function( jqXHR, textStatus, errorThrown ) {	console.log("BAD "+textStatus);	});
			})
...
<body>
		...
		<div id="content-sidebar_popup" class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
			<form method="post" name="form_loginadmin" action="">
				...
				<div id="container_credit" class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
				    ...
				    <label class="label label_blue_sky" for="txt_limitcredit">L. de Cr&eacute;dito</label>
					...
					<div id="container_plazo" class="col-xs-2 col-sm-2 col-md-1 col-lg-1">
				    <label class="label label_blue_sky" for="txt_credit_term">Plazo</label>
					...
					<div id="container_saldo" class="col-xs-3 col-sm-3 col-md-3 col-lg-3">
						<div id="container_txtsaldo" class="col-xs-6 col-sm-6 col-md-6 col-lg-6">
							<label class="label label_blue_sky" for="span_saldo">Saldo </label>
					...
					<div class="col-xs-2 col-sm-2 col-md-2 col-lg-2">
						<label for="cb_restricted" class="label label_blue_sky">Restringido</label>
						<label id="lbl_cb_restricted" class="switch">
							<?php $restricted = ($rs_client['TX_cliente_restringido'] === "1") ? 'checked' : '' ?>
							<input id="cb_restricted" type="checkbox" <?php echo $restricted; ?> >
							<span class="slider_switch round"></span>
						</label>
					</div>
					<div class="col-xs-2 col-sm-2 col-md-2 col-lg-2">
						<label for="cb_exento" class="label label_blue_sky">Exento</label>
						<label id="lbl_cb_exento" class="switch">
							<?php $exento = ($rs_client['TX_cliente_exento'] === "1") ? 'checked' : '' ?>
							<input id="cb_exento" type="checkbox" <?php echo $exento; ?> >
							<span class="slider_switch round"></span>
						</label>
					</div>
	...
	<?php			$date_i=date('d-m-Y',strtotime($ff_oldest));
						$date_f=date('d-m-Y');	?>
	...
	<?php 		$date_i=date('Y-m-d',strtotime($date_i));
		    					...
		    					while($rs_facturaf=$qry_facturaf->fetch_array()){ ?>
								    <tr title="<?php echo $rs_facturaf['TX_user_seudonimo']; ?>">
								    	...
								    	<td><?php echo $rs_facturaf['TX_facturaf_hora']; ?></td>
											...
											<td><?php echo $rs_facturaf['TX_facturaf_ticket']; ?></td>
	...
	<?php 	$date_i=date('d-m-Y',strtotime('-1 week'));
					$date_f=date('d-m-Y');
	?>			<div id="container_filternc" class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
				    ...
				    <div id="container_btnfilter" class="col-xs-2 col-sm-2 col-md-2 col-lg-2 side-btn-md-label">
			        <button type="button" id="btn_filter_nc" class="btn btn-success"><i class="fa fa-search" aria-hidden="true"></i></button>
				    </div>
					</div>
	...
	?>			<div id="container_filternc" class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
				    ...
				    <div id="container_btnfilter" class="col-xs-2 col-sm-2 col-md-2 col-lg-2 side-btn-md-label">
				      <button type="button" id="btn_filter_pay" class="btn btn-success"><i class="fa fa-search" aria-hidden="true"></i></button>
				    </div>
					</div>
	...
	<?php 				$total_payment=0;
								if($nr_payment=$qry_payment->num_rows > 0){
									while($rs_payment=$qry_payment->fetch_array()){
										...
										INNER JOIN bh_facturaf ON bh_facturaf.AI_facturaf_id = rel_facturaf_notadebito.rel_AI_facturaf_id) WHERE AI_notadebito_id = '{$rs_payment[2]}'")or die($link->error);
										$ff="";
