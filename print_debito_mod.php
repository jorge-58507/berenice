...
<?php 	$raw_facturaf=array();
				do{
					$raw_facturaf[] = $rs_facturaf['AI_facturaf_id'];
?>
        <tr>
			...
			<?php }while($rs_facturaf=$qry_facturaf->fetch_array()); echo json_encode($raw_facturaf);?>
        </tbody>
	</table>
<?php
	...
	foreach ($raw_facturaf as $key => $value) {
		$qry_nd_afected->bind_param("i",$value);
		$qry_nd_afected->execute();
		$result = $qry_nd_afected->get_result();
		while ($rs_nd_afected=$result->fetch_array()) {
			if (!in_array($rs_nd_afected['rel_AI_notadebito_id'],$raw_nd)) {
				$raw_nd[$value]=$rs_nd_afected['rel_AI_notadebito_id'];
			}
		}
	}
	$qry_datodebito = $link->prepare("SELECT datodebito_AI_metododepago_id
		FROM (bh_datodebito
	...
	<table align="center" border="1" cellpadding="0" cellspacing="0" style="width:100%; font-size:12px; text-align:center;">
		...
		<thead>
			<tr>
				...
				<th style="width:40%">Fact. Afectada</th>
			</tr>
		</thead>
		<tbody>
			<tr>
