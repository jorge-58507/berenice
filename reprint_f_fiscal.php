...
$facturaf_id = $raw_generate['factid'];
...
$txt_datopago="SELECT bh_datopago.TX_datopago_monto, bh_datopago.TX_datopago_numero, bh_datopago.datopago_AI_metododepago_id FROM bh_datopago WHERE bh_datopago.datopago_AI_facturaf_id = '$facturaf_id'";
...
while($rs_datopago=$qry_datopago->fetch_array()){
	...
	$raw_datopago[$rs_datopago['datopago_AI_metododepago_id']] = ['monto' => $rs_datopago['TX_datopago_monto'], 'numero' => $rs_datopago['TX_datopago_numero']];
...
if ($flete === "1") {
  ...
  $str_facmv .= "FACTI".substr($rs_facturaf['TX_facturaf_numero'],-8).chr(9).$codigo.chr(9).trim("Entregar en: ".$descripcion).chr(9)."UNIDADES".chr(9)."1".chr(9)."0.00".chr(9)."0".chr(9). PHP_EOL;
}
