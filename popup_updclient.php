			...
			$('#btn_acept').click(function(){
				...
				if ($('#sel_client_type').val() != "2") {
					...
					if (is_empty_var($("#txt_cif").val()) === 0 || $("#txt_cif").val().length < 5	|| ruc.charAt(0) == 0 && ruc.charAt(1) == '-' || ruc.charAt(0) == 0 && ruc.charAt(1) == 0) {
						shot_snackbar('Debe ingresar documento de 5 d&iacute;gitos.');
						$("#txt_cif").focus();	return false;
					}
				...
				$.ajax({
					data: { "a" : name, "b" : $("#txt_cif").val(), "c" : $("#txt_telephone").val(), "d" : $("#txt_direction").val(), "e" : '<?php echo $client_id; ?>', "f" : activo, "g" : $("#txt_dv").val(), "h" : pending, "i" : $('#sel_client_taxpayer').val(), "j" :  $('#sel_client_type').val(), "k" :  $('#txt_client_email').val() },
					type: "GET",	
					dataType: "text",	
				...
				.done(function( data, textStatus, jqXHR ) {
					if (activo != '') {
						window.opener.$("#container_txtfilterclient"+"_"+activo).html(data);
					} else {
